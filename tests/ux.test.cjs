...
test('compact create keeps optional values and exposes their summary when collapsed', t => {
  ...
  assert.match(p.data.moreSummary,/21:30.*2分钟.*明天/); assert.equal(h.store.read().habits.length,0);
});
test('Unicode title cap agrees with domain and validation errors are next to fields', t => {
  ...
  assert.equal(p.data.moreOpen,true); assert.ok(p.data.fieldErrors.minimum); assert.equal(h.store.read().habits.length,0);
});
...
test('registered pages, seven-column selectors, truthful copy and data-menu placement are wired', () => {
  ...
  assert.match(read('app.wxss'),/\.week-options\s*\{[^}]*repeat\(7, minmax\(0, 1fr\)\)/);
  ...
  assert.match(read('templates/task.wxml'),/>已完成</); assert.match(read('templates/task.wxml'),/>撤销打卡</);
});
